...
class KirchhoffPD:
    def __init__(self, material, plate_config, sim_config, load_config, bc_config):
        ...
        self.disp_z = tf.Variable(self.bc_config.disp_BC, dtype=DTYPE)

        # initial coordinate
        # self.init_coord = self.prepare_coord_array()
        self.init_coord = tf.constant(self.prepare_coord_array(), dtype=DTYPE)
    ...
    def run_static(self, safety_factor=0.5, output_dir="output"):
        ...
        time = tf.Variable(0.0, dtype=DTYPE)
        dt = tf.constant(self.sim_config.dt, dtype=DTYPE)
        ...
        # disp_z_old = tf.Variable(np.zeros_like(self.disp_z), dtype=DTYPE)
        pd_forces_old = tf.Variable(np.zeros_like(body_forces), dtype=DTYPE) 

        velhalf = tf.Variable(np.zeros_like(self.disp_z), dtype=DTYPE)
        velhalf_old = tf.Variable(np.zeros_like(self.disp_z), dtype=DTYPE)
    ...
    def ADR_time_integration(self, disp_z, velhalf, velhalf_old,
        ...
        """ 
            ...
            pd_force_old:
        """
        emod   = tf.constant(self.material.youngs_modulus, dtype=DTYPE)
        pratio = tf.constant(self.material.poissons_ratio, dtype=DTYPE)
        thick  = tf.constant(self.plate_config.thickness, dtype=DTYPE)
        delta  = tf.constant(self.plate_config.horizon, dtype=DTYPE)
        dx     = tf.constant(self.plate_config.dx, dtype=DTYPE)
    ...
    def calc_PD_force(self):
        ...
        dx = tf.constant(self.plate_config.dx          , dtype=DTYPE)
        horizon = tf.constant(self.plate_config.horizon, dtype=DTYPE)
        thick = tf.constant(self.plate_config.thickness, dtype=DTYPE)
        PI = tf.constant(np.pi                         , dtype=DTYPE)
        ...
        flectural_rigidity = tf.constant(
                ...
                (12.0 * (1.0-self.material.poissons_ratio**2)), dtype=DTYPE)

        # print("flectural_rigidity= {:g}".format(flectural_rigidity))
        # poisson's ratio
        nu = tf.constant(self.material.poissons_ratio  , dtype=DTYPE)
        # every volume is same in linear grid model.
        vol = tf.constant(self.plate_config.vol        , dtype=DTYPE)

        # smooth factor for j
        # smooth factor for i_k, i_j
        smooth_fac_kernel = tf.constant(self.calc_dist_smooth_fac()[np.newaxis, :, :, np.newaxis], dtype=DTYPE)
        ...
        # angle for j
        # angle for i_k, i_j
        angle_kernel = tf.constant(self.calc_angle()[np.newaxis, :, :, np.newaxis] , dtype=DTYPE)
        angle_i = tf.reshape(angle_kernel, shape=(1, 1, 1, LEN_i))

        # dist^2 for j
        # dist^2 for i_k, i_j
        dist_pow2_kernel = tf.constant(self.calc_dist_pow2()[np.newaxis, :, :, np.newaxis], dtype=DTYPE)
        ...
        # forces = tf.cast(disp_z_k_j_i[:, KERNEL_SIZE//2, KERNEL_SIZE//2, tf.newaxis, tf.newaxis, :], DTYPE) -  \
        # tf.cast(disp_z_k_j_i[:, KERNEL_SIZE//2, KERNEL_SIZE//2,  tf.newaxis, tf.newaxis, tf.newaxis, LEN_i//2], DTYPE)
        # forces = angle_i
    ...
    def apply_disp_BC(self):
        ...
        self.disp_z = tf.Variable(np_disp, dtype=DTYPE)
